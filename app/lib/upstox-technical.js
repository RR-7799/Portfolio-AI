...
async function getMarketData(key,toDate,fromDate,token){
 const q3=await fetchUpstox(UPSTOX_V3,`/market-quote/ltp?instrument_key=${encodeURIComponent(key)}`,token);
 const h3=await fetchUpstox(UPSTOX_V3,`/historical-candle/${encodeURIComponent(key)}/days/1/${toDate}/${fromDate}`,token);
 let q=q3,h=h3,source="v3";
 const candles3=h3.body?.data?.candles||[];
 if(!q3.ok||candles3.length<30){
   const q2=await fetchUpstox(UPSTOX_V2,`/market-quote/ltp?instrument_key=${encodeURIComponent(key)}`,token);
   const h2=await fetchUpstox(UPSTOX_V2,`/historical-candle/${encodeURIComponent(key)}/day/${toDate}/${fromDate}`,token);
...
export async function getTechnicalForIsin(isin,days=365){
 ...
 const {q,h,source}=await getMarketData(key,toDate,fromDate,token);
 const qt=quote(q.body,key),candles=h.body?.data?.candles||[],closes=candles.map(x=>n(x?.[4])).filter(x=>x!=null),highs=candles.map(x=>n(x?.[2])).filter(x=>x!=null),lows=candles.map(x=>n(x?.[3])).filter(x=>x!=null),volumes=candles.map(x=>n(x?.[5])).filter(x=>x!=null),price=qt.price??closes.at(-1)??null;
 if(!q.ok&&!h.ok) return{available:false,reason:`Upstox market data unavailable (quote ${q.status}, history ${h.status}).`,status:"UPSTOX_UNAVAILABLE",price,quote_status:q.status,historical_status:h.status,candles:closes.length,source};
 if(!price)return{available:false,reason:"No current or historical price returned by Upstox.",status:"PRICE_UNAVAILABLE",price:null,quote_status:q.status,historical_status:h.status,candles:closes.length,source};
 if(closes.length<30)return{available:false,reason:"Insufficient daily candle history for technical scoring.",status:"INSUFFICIENT_HISTORY",price,previous_close:qt.previous_close,quote_status:q.status,historical_status:h.status,candles:closes.length,source};
