...
const scoreClass = (action) =>
  ["BUY", "ACCUMULATE"].includes(action) ? "positive" : ["EXIT", "SELL", "REDUCE"].includes(action) ? "negative" : "";
...
export default function PositionIntelligence() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    supabase.from("instruments").select("id,company_name,symbol").then(({ data }) => {
      if (mounted) setInstruments(data || []);
    });
    return () => { mounted = false; };
  }, [session]);
  ...
  useEffect(() => {
    const handler = async (event) => {
      const row = event.target.closest("section table tbody tr");
      if (!row) return;

      const section = row.closest("section");
      const heading = Array.from(section?.querySelectorAll("span,h2,h3") || [])
        .find((node) => String(node.textContent || "").trim().toUpperCase() === "STOCK HOLDINGS");
      if (!heading) return;

      const cells = Array.from(row.children);
      const text = cells.map((cell) => String(cell.textContent || "").trim());
      const candidates = instruments.filter((x) =>
        text.some((value) => value && (value === String(x.symbol || "") || value === String(x.company_name || "")))
      );
  ...
  return (
    <div
      ...
      style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(15,23,42,.28)", display: "flex", justifyContent: "flex-end" }}
    >
      <aside
        onClick={(e) => e.stopPropagation()}
        style={{ width: "min(520px, 94vw)", height: "100%", background: "white", overflowY: "auto", padding: "28px", boxShadow: "-12px 0 40px rgba(15,23,42,.16)" }}
      >
        ...
        {!loading && !error && data && (
          <>
            ...
            <div style={{ marginTop: 22 }}>
              ...
              <p style={{ lineHeight: 1.55 }}>{score.ai_summary || "The engine has not generated a detailed summary for this position yet."}</p>
            </div>
            ...
            <div className="card" style={{ marginTop: 22 }}>
              ...
              <div style={{ marginTop: 8, fontWeight: 700 }}>{data.market_regime?.regime || "—"}</div>
              <div style={{ opacity: .65, marginTop: 3 }}>{data.market_regime?.portfolio_mode || "—"}</div>
            </div>
          </>
        )}
      </aside>
