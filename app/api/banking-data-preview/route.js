...
function auth(request) {
  ...
  return !!secret && (header === secret || bearer === `Bearer ${secret}`);
}
...
function collectLabels(node, path = [], out = []) {
  if (Array.isArray(node)) {
    node.forEach((item, index) => collectLabels(item, [...path, String(index)], out));
    return out;
  }
  if (!node || typeof node !== "object") return out;
  if (node.particular != null) {
    const label = String(node.particular);
    out.push({ label, normalized: normalize(label), path });
  }
  for (const [key, value] of Object.entries(node)) {
    if (key !== "history") collectLabels(value, [...path, key], out);
  }
  return out;
}
...
function extractHistory(node, wanted, out = []) {
  ...
  if (label && wanted.some(term => label === normalize(term) || label.includes(normalize(term)))) {
    if (Array.isArray(node.history)) {
      out.push({ particular: node.particular, history: node.history });
    }
  }
  for (const [key, value] of Object.entries(node)) {
    if (key !== "history") extractHistory(value, wanted, out);
  }
  return out;
}

export async function GET(request) {
  if (!auth(request)) return NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 });
  try {
    const results = [];
    for (const target of TARGETS) {
      ...
      results.push({
        ...
        available_line_items: collectLabels(fullStatement).map(x => x.label),
        candidate_banking_items: extractHistory(fullStatement, [
