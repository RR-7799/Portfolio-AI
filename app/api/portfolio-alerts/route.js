...
const ENGINE_VERSION = "portfolio_alerts_v1_9";
  ...
  for(const x of holdings){
   ...
   if(prior){
    ...
    if(action!==priorAction&&action){
      const escalation=["EXIT","REDUCE"].includes(action);const improvement=["BUY","ACCUMULATE"].includes(action);
      if(escalation)alerts.push({user_id:userId,instrument_id:aid,severity:action==="EXIT"?"CRITICAL":"WARNING",type:"ACTION_CHANGE",title:`${name} action changed`,message:`AI action moved from ${priorAction||"UNKNOWN"} to ${action}.`,dedupe_key:`ACTION_CHANGE:${x.instrument_id}:${priorAction}:${action}`});
      else if(improvement&&scoreDelta>=5)alerts.push({user_id:userId,instrument_id:aid,severity:"INFO",type:"ACTION_CHANGE",title:`${name} action improved`,message:`AI action moved from ${priorAction||"UNKNOWN"} to ${action} with AI score improving by ${scoreDelta.toFixed(1)} points.`,dedupe_key:`ACTION_CHANGE:${x.instrument_id}:${priorAction}:${action}`});
    }
