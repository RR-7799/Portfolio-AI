...
export async function GET(request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, calculation_scope: "CURRENT_TRADING_DAY", trading_date_ist: date, trades: rows, summary: { total: rows.length, open, closed: closed.length, wins, losses: closed.filter(r => Number(r.realized_r) < 0).length, net_r: Number(netR.toFixed(3)), win_rate: closed.length ? Number(((wins / closed.length) * 100).toFixed(1)) : null } });
  } catch (error) {
