...
const ENGINE_VERSION = "final_decision_v1_1";
...
function decisionFor({ fundamentalScore, technicalScore, risk, freshness, confidence, weight }) {
  ...
  if (fr === "MISSING" || fr === "VERY_STALE") return { decision: blended >= 75 ? "WATCH" : "HOLD", conviction: "LOW", score: blended, reason: "Fundamental data freshness is too weak for a high-conviction action." };
  if (r === "HIGH" && blended < 80) return { decision: blended < 55 ? "EXIT" : "REDUCE", conviction: "MEDIUM", score: blended, reason: "Elevated model risk outweighs the current combined score." };
...
export async function GET(request) {
  try {
    ...
    if (!stockRes.ok || !stockRes.body?.success) return NextResponse.json({ success: false, engine_version: ENGINE_VERSION, error: stockRes.body?.error || "Unable to load stock intelligence." }, { status: stockRes.status || 502 });
    ...
    const holdingRows = stock.portfolio?.current_value != null ? null : null;
    ...
    const result = decisionFor({ fundamentalScore, technicalScore, risk: fundamentals.risk_level, freshness: freshness.status, confidence: breakdown.confidence ?? freshness.effective_confidence, weight });
    ...
    return NextResponse.json({
      ...
      fundamentals: { score: fundamentalScore, action: fundamentals.action || null, rating: fundamentals.rating || null, risk: fundamentals.risk_level || null, confidence: breakdown.confidence ?? freshness.effective_confidence ?? null, freshness: freshness.status || null },
      technicals: market ? { score: technicalScore, trend: market.technical?.trend || null, price: market.technical?.price ?? null, change_pct: market.technical?.change_pct ?? null, rsi14: market.technical?.momentum?.rsi14 ?? null, momentum_3m: market.technical?.momentum?.three_month ?? null, momentum_1y: market.technical?.momentum?.one_year ?? null, entry_zone: market.technical?.trade_plan?.entry_zone || null, stop_loss: market.technical?.trade_plan?.stop_loss ?? null, target_1: market.technical?.trade_plan?.target_1 ?? null, target_2: market.technical?.trade_plan?.target_2 ?? null, risk_reward: market.technical?.trade_plan?.risk_reward_to_target_1 ?? null } : null,
      final: { decision: result.decision, conviction: result.conviction, score: Number(result.score.toFixed(1)), position_weight_pct: weight === null ? null : Number(weight.toFixed(2)), reason: result.reason },
