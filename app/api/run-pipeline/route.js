...
async function parseResponse(response) {
  try {
    ...
    return { success: false, error: "Pipeline stage returned non-JSON response", status: response.status };
  }
}
...
async function runPipelineStage(label, handler, url, headers = {}) {
  ...
  try {
    ...
    return {
      ...
      duration_ms: Date.now() - started,
      success: false,
      data: { success: false, error: error?.message || `${label} failed` },
    };
  }
}
...
export async function GET(request) {
    ...
    return NextResponse.json(
      {
        ...
        error: process.env.PIPELINE_SECRET ? "Unauthorized" : "PIPELINE_SECRET is not configured",
      },
      { status: 401 }
    );
  }
  ...
  const selectedStage = ["sync", "freshness", "score", "snapshot", "alerts", "all"].includes(requestedStage)
    ? requestedStage
    : "all";
  ...
  try {
    ...
    if (selectedStage === "score" || selectedStage === "all") {
      const result = await runPipelineStage("ai_scorer_v5_1", runV51, `${origin}/api/calculate-score`, headers);
      stages.push(result);
      if (!result.success) {
        return NextResponse.json(
          {
    ...
    if (selectedStage === "alerts" || selectedStage === "all") {
      stages.push(await runPipelineStage("portfolio_alerts", runAlerts, `${origin}/api/portfolio-alerts`, headers));
    }
    ...
    const score = stages.find((item) => item.stage === "ai_scorer_v5_1");
    const fresh = stages.find((item) => item.stage === "freshness_audit");
    const snapshot = stages.find((item) => item.stage === "portfolio_snapshot");
    const alerts = stages.find((item) => item.stage === "portfolio_alerts");
    const alertsWarning = alerts && !alerts.success
      ? alerts.data?.error || alerts.data?.message || `Alert stage failed with HTTP ${alerts.http_status}.`
      : alerts?.data?.warning || null;
    ...
    return NextResponse.json({
      ...
      pipeline_summary: {
        ...
        sync_successful: syncResults.filter((item) => item.success === true).length,
        sync_failed: syncResults.filter((item) => item.success === false && item.skipped !== true).length,
        freshness_completed: Boolean(fresh?.success),
        scoring_completed: Boolean(score?.success),
        score_history_completed: Boolean(score?.success && score?.data?.summary?.history_stored > 0),
        score_history_stored: score?.data?.summary?.history_stored ?? null,
        score_history_failed: score?.data?.summary?.history_failed ?? null,
        ...
        buy_candidates: score?.data?.results?.filter?.((item) => item.action === "BUY")?.length ?? null,
        average_score: score?.data?.summary?.average_final_ai_score ?? null,
      },
      stages,
    });
  } catch (error) {
