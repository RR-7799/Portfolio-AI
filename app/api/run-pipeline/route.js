...
const ENGINE_VERSION = "pipeline_v1_2";
const BATCH_SIZE = 10;
...
export async function GET(request){
 ...
 try{
  ...
  if(stage==="alerts"||stage==="all"){const r=await runStage("portfolio_alerts",runAlerts,`${origin}/api/portfolio-alerts`,headers);stages.push(r);if(!r.success)return NextResponse.json({success:false,engine_version:ENGINE_VERSION,failed_stage:r.stage,elapsed_ms:Date.now()-startedAt,stages},{status:502});}
