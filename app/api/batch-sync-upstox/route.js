...
const ENGINE_VERSION = "upstox_batch_v1_0";
...
async function syncSingleInstrument(baseUrl, isin) {
  const url =
    `${baseUrl}/api/sync-upstox-fundamentals?isin=` +
  ...
  try {
    ...
    try {
      ...
      data = {
        success: false,
        error: "Sync endpoint returned non-JSON response",
...
export async function GET(request) {
  ...
  try {
    ...
    /*
      IMPORTANT:
      symbol currently stores ISINs for the imported instruments.
      We therefore use symbol as the Upstox identifier.
    */
    ...
    const stockInstruments = uniqueInstruments.filter(
      (instrument) =>
        instrument?.symbol &&
        !isMutualFund(instrument)
    );
    ...
    for (const instrument of batch) {
      ...
      /*
        Only process ISIN-looking identifiers.
        This protects the batch engine from accidentally sending
        ticker symbols or malformed values to the Upstox ISIN endpoint.
      */
    ...
    const failed = results.filter(
      (item) => item.success === false && item.skipped !== true
    );
    ...
    const partial = successful.filter(
      (item) =>
        item.completeness !== null &&
        Number(item.completeness) < 100
    );

    const complete = successful.filter(
      (item) =>
        item.completeness !== null &&
        Number(item.completeness) >= 100
    );
    ...
    const nextOffset =
      offset + batch.length < totalStocks
        ? offset + batch.length
        : null;
