...
const ENGINE_VERSION = "intraday_engine_v1_4";
...
async function loadNifty500Symbols() {
  const r = await fetch(NIFTY_500, { cache: "no-store", headers: { Accept: "text/csv,*/*" } });
  if (!r.ok) throw new Error(`NIFTY 500 constituent list unavailable (${r.status}).`);
  const rows = parseCsv(await r.text());
  const headerIndex = rows.findIndex(row => row.some(cell => /symbol/i.test(cell)));
  if (headerIndex < 0) throw new Error("NIFTY 500 constituent list has no Symbol column.");
  const header = rows[headerIndex].map(x => x.toLowerCase());
  const symbolIndex = header.findIndex(x => x === "symbol" || x.includes("symbol"));
  if (symbolIndex < 0) throw new Error("NIFTY 500 constituent list has no usable Symbol column.");
  const symbols = new Set(rows.slice(headerIndex + 1).map(row => String(row[symbolIndex] || "").trim().toUpperCase()).filter(Boolean));
  if (symbols.size < 490 || symbols.size > 510) throw new Error(`NIFTY 500 source returned ${symbols.size} symbols; refusing an incomplete universe.`);
  return symbols;
}
async function loadUniverse() {
  const [instrumentResponse, indexSymbols] = await Promise.all([fetch(NSE_INSTRUMENTS, { cache: "no-store" }), loadNifty500Symbols()]);
  ...
  if (selected.length < 490 || selected.length > 510) throw new Error(`NIFTY 500 mapping produced ${selected.length} NSE equity instruments; refusing an incomplete universe.`);
  return selected;
}
function quoteMap(body) {
  ...
  for (const [key, row] of Object.entries(body?.data || {})) {
    const q = row || {};
    out.set(key.replace(":", "|"), { price: n(q.last_price), prev: n(q.prev_close_price ?? q.cp), volume: n(q.ohlc?.volume ?? q.volume) });
  }
  return out;
}
async function intradayMetrics(key, token) {
  ...
  const last = completed.at(-1); const price = n(last?.[4]);
  const recentVol = vols.at(-1); const baseline = avg(vols.slice(Math.max(0, vols.length - 21), -1));
  ...
  const priorHigh = Math.max(...highs.slice(-13, -1)); const priorLow = Math.min(...lows.slice(-13, -1));
  const recentLow3 = Math.min(...lows.slice(-3)); const recentHigh3 = Math.max(...highs.slice(-3));
  ...
  return { price, relativeVolume, vwap, priorHigh, priorLow, recentLow3, recentHigh3, momentum5, candleBodyPct, candleDirection };
}
function rankCandidate(meta, quote, m) {
  if (!quote?.price || !m?.price || !m.vwap) return null;
  const entry = quote.price; const changePct = quote.prev ? ((entry - quote.prev) / quote.prev) * 100 : 0; const rv = m.relativeVolume ?? 0;
  const vwapDistancePct = Math.abs((entry - m.vwap) / m.vwap) * 100; const aboveVwap = entry > m.vwap;
  const breakoutLong = entry > m.priorHigh && m.candleDirection === "LONG" && m.candleBodyPct >= 0.15 && rv >= 1.5;
  const breakoutShort = entry < m.priorLow && m.candleDirection === "SHORT" && m.candleBodyPct >= 0.15 && rv >= 1.5;
  const longMomentum = changePct > 0.6 && m.momentum5 > 0.3 && aboveVwap && m.candleDirection === "LONG" && rv >= 1.5;
  const shortMomentum = changePct < -0.6 && m.momentum5 < -0.3 && !aboveVwap && m.candleDirection === "SHORT" && rv >= 1.5;
  ...
  if (vwapDistancePct > (breakout ? 1.0 : 0.8)) return null;
  if (breakout && Math.abs((entry - (direction === "LONG" ? m.priorHigh : m.priorLow)) / entry) * 100 > 0.45) return null;
  let score = 45 + Math.min(20, Math.abs(changePct) * 8) + Math.min(20, Math.max(0, rv - 1) * 12);
  score += aboveVwap === (direction === "LONG") ? 10 : 0; score += breakout ? 10 : 0; score += m.candleBodyPct >= 0.3 ? 5 : 0;
  score = Math.max(0, Math.min(100, score)); if (score < MIN_SETUP_SCORE) return null;
  const stop = direction === "LONG" ? m.recentLow3 : m.recentHigh3; const risk = Math.abs(entry - stop); const riskPct = (risk / entry) * 100;
  if (!Number.isFinite(risk) || risk <= 0 || riskPct < MIN_RISK_PCT || riskPct > MAX_RISK_PCT) return null;
  const target1 = direction === "LONG" ? entry + risk * 2 : entry - risk * 2; const target2 = direction === "LONG" ? entry + risk * 3 : entry - risk * 3;
  return { instrument_key: meta.instrument_key, trading_symbol: meta.trading_symbol, company_name: meta.name, price: entry, change_pct: changePct, volume: quote.volume, relative_volume: rv, vwap: m.vwap, setup_score: Number(score.toFixed(1)), setup, direction, entry_price: entry, stop_loss: stop, target_1: target1, target_2: target2, risk_reward: 2, risk_pct: Number(riskPct.toFixed(3)), data_status: "LIVE" };
}
export async function GET(request) {
  ...
  try {
    const universe = await loadUniverse(); const quotes = new Map();
    for (let i = 0; i < universe.length; i += 500) {
      const keys = universe.slice(i, i + 500).map(x => x.instrument_key).join(","); const r = await upstox(`/market-quote/quotes?instrument_key=${encodeURIComponent(keys)}`, token);
    ...
    candidates.sort((a, b) => b.setup_score - a.setup_score || a.risk_pct - b.risk_pct); const top = candidates.slice(0, 15); const scannedAt = new Date().toISOString();
    const supabase = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY); let tracked = 0;
    if (top.length) {
      ...
      if (data?.length) {
        ...
        if (trackedResult.error) throw trackedResult.error; tracked = data.length;
      }
    }
    return NextResponse.json({ success: true, engine_version: ENGINE_VERSION, universe: "NIFTY_500", universe_count: universe.length, quote_count: quotes.size, intraday_candidates_scanned: universe.length, signal_count: top.length, tracked_signal_count: tracked, risk_policy: { min_setup_score: MIN_SETUP_SCORE, min_risk_pct: MIN_RISK_PCT, max_risk_pct: MAX_RISK_PCT, target1_r: 2, target2_r: 3, stop_policy: "recent_3_candle_structure", max_vwap_extension_pct: 1.0 }, signals: top, scanned_at: scannedAt, elapsed_ms: Date.now() - started });
  } catch (error) {
