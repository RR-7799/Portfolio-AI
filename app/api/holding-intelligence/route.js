...
const ENGINE_VERSION = "holding_intelligence_v1_1";
...
function evidenceFromScore(score) {
  ...
  return Object.entries(labels)
    .map(([key, factor]) => ({ factor, score: n(score?.[key]) }))
...
export async function GET(request) {
  try {
    ...
    const weaknesses = evidence.filter((x) => x.score < 60).slice(0, 4);

    const invalidation = [];
    if (score.total_score != null) invalidation.push(`AI score falls below ${score.total_score >= 75 ? 75 : 60}.`);
    if (["HIGH", "CRITICAL"].includes(String(score.risk_level || "").toUpperCase())) invalidation.push("Risk remains elevated or deteriorates further.");
