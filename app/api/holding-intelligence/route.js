...
export async function GET(request){
  try{
    ...
    const [h,i,s,mr]=await Promise.all([
      ...
      client.from("ai_scores").select("instrument_id,long_term_score,short_term_score,final_ai_score,risk_score,valuation_score,confidence,data_completeness,freshness_status,score_version,rating,action,risk_level,ai_summary,score_breakdown,score_date,calculated_at,updated_at").eq("instrument_id",instrumentId).eq("user_id",authData.user.id).eq("score_version",SCORE_VERSION).maybeSingle(),
    ...
    if(!s.data)return NextResponse.json({success:false,engine_version:ENGINE_VERSION,error:"No current V5.5 score is available for this holding."},{status:404});
