...
const ENGINE_VERSION = "freshness_v1_0";
...
function classifyFreshness(ageMonths) {
  if (ageMonths === null) {
    return {
      ...
      confidence: 0,
      maxScore: 49,
      reason: "No financial statement period is available.",
    };
  }
  ...
  if (ageMonths <= 12) {
    return {
      ...
      confidence: 100,
      maxScore: 100,
      reason: "Financial data is within the last 12 months.",
    };
  }
  ...
  if (ageMonths <= 18) {
    return {
      ...
      confidence: 80,
      maxScore: 84,
      reason: "Financial data is older than one year but within 18 months.",
    };
  }
  ...
  if (ageMonths <= 24) {
    return {
      ...
      confidence: 60,
      maxScore: 74,
      reason: "Financial data is between 19 and 24 months old.",
    };
  }
  ...
  if (ageMonths <= 36) {
    return {
      ...
      confidence: 35,
      maxScore: 69,
      reason: "Financial data is more than two years old.",
    };
  }
  ...
  return {
    ...
    confidence: 15,
    maxScore: 59,
    reason: "Financial data is more than three years old.",
  };
}
...
export async function GET() {
  try {
    ...
    if (holdingsError) {
      throw new Error(`Holdings query failed: ${holdingsError.message}`);
    }

    const instrumentIds = [
      ...new Set((holdings || []).map((row) => row.instrument_id).filter(Boolean)),
    ];
    ...
    const [{ data: instruments, error: instrumentsError }, { data: fundamentals, error: fundamentalsError }] = await Promise.all([
      ...
      supabase
        .from("fundamentals")
        .select("instrument_id,financial_year,shareholding_date,updated_at")
        .in("instrument_id", instrumentIds),
    ]);

    if (instrumentsError) {
      throw new Error(`Instruments query failed: ${instrumentsError.message}`);
    }

    if (fundamentalsError) {
      throw new Error(`Fundamentals query failed: ${fundamentalsError.message}`);
    }
    ...
    for (const item of results) {
      freshnessCounts[item.freshness_status] = (freshnessCounts[item.freshness_status] || 0) + 1;
    }
