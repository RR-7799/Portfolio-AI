...
const ENGINE_VERSION = "market_regime_v1_0";
...
function buildIndexMetrics(body, quote) {
  ...
  return {
    ...
    momentum: {
      three_month_return_pct: return3m,
      one_year_return_pct: return1y,
    },
    candles: closes.length,
  ...
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { success: false, engine_version: ENGINE_VERSION, error: "UPSTOX_ANALYTICS_TOKEN is missing." },
        { status: 500 }
      );
    }
    ...
    const responses = await Promise.all(
      jobs.map(async ([name, type, path]) => [
        name,
        type,
        await upstoxFetch(path, token),
      ])
    );
    ...
    for (const [name, key] of Object.entries(SOURCES)) {
      ...
      indicators[name] = buildIndexMetrics(
        item.history?.body,
        parseQuote(item.quote?.body, key)
      );
    ...
    if (nifty?.available && bank?.available) {
      baseScore = nifty.regime_score * 0.6 + bank.regime_score * 0.4;
    }
