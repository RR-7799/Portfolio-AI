...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({ success: true, engine_version: ENGINE_VERSION, user_id: data.user.id, scanned_at: new Date().toISOString(), pipeline_summary: { ...baseSummary, alerts_completed: alertOnlyFailure ? false : baseSummary.alerts_completed, alerts_warning: pipelineWarning, decision_engine_completed: true, decision_engine_version: decisionPayload.engine_version || null, decision_counts: decisionCounts, buy_candidates: Number(decisionCounts.BUY || 0), coverage }, decision_engine: { engine_version: decisionPayload.engine_version || null, decision_counts: decisionCounts } });
