...
export async function GET(request) {
  ...
  try {
    ...
    const { data, error } = await db.from("intraday_signal_outcomes").select("direction,setup,setup_score,outcome,realized_r,mae_pct,mfe_pct,signal_at,closed_at,status,horizon_10m_directional,horizon_20m_directional,horizon_30m_directional").eq("user_id", user.id).order("signal_at", { ascending: false }).limit(5000);
