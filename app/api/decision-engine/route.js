...
function percentile(value, peers){
  const v=n(value), p=(peers||[]).map(n).filter(x=>x!==null).sort((a,b)=>a-b);
...
function peerContext(row, peers){
  const quality = x => avg([pct(x.profitability_score,20),pct(x.debt_score,10),pct(x.cashflow_score,10),pct(x.ownership_score,10)]);
  return {
    valuation_pct:percentile(row.valuation_score,peers.map(x=>x.valuation_score)),
    growth_pct:percentile(row.growth_score,peers.map(x=>x.growth_score)),
    quality_pct:percentile(quality(row),peers.map(quality)),
    score_pct:percentile(row.total_score,peers.map(x=>x.total_score))
  };
}
function decisionV3(input){
  ...
  const s=n(score)||0,r=up(risk),f=up(freshness),m=up(regime);
  ...
  const stale=["MISSING","VERY_STALE"].includes(f);
  const severeDrawdown=(n(pnl)||0)<=-25;
  const overweight=(n(weight)||0)>=15;
  ...
  else if(s>=82&&quality>=65&&!criticalRisk&&!highRisk&&!stale&&!relativelyExpensive&&m!=="BEAR"&&(n(weight)||0)<12&&(relativeStrength===null||relativeStrength>=65)){action="BUY";reason="Strong absolute score plus strong relative peer position and acceptable portfolio weight";confidence=90;}
  ...
  return {
    ...
    diagnostics:{
      thesis_score:+thesis.toFixed(1),quality_score:+quality.toFixed(1),
      relative_strength:relativeStrength===null?null:+relativeStrength.toFixed(1),
      relative_value_percentile:relativeValue===null?null:+relativeValue.toFixed(1),
      relative_growth_percentile:peer?.growth_pct===null?null:+peer.growth_pct.toFixed(1),
      relative_quality_percentile:peer?.quality_pct===null?null:+peer.quality_pct.toFixed(1),
      normalized_components:{growth:growthP===null?null:+growthP.toFixed(1),profitability:profitP===null?null:+profitP.toFixed(1),debt:debtP===null?null:+debtP.toFixed(1),ownership:ownershipP===null?null:+ownershipP.toFixed(1),cashflow:cashP===null?null:+cashP.toFixed(1),valuation:valuationP===null?null:+valuationP.toFixed(1),risk:riskP===null?null:+riskP.toFixed(1)},
      signals:{strong_thesis:strongThesis,broken_thesis:brokenThesis,critical_risk:criticalRisk,high_risk:highRisk,overweight,severe_drawdown:severeDrawdown,relatively_cheap:relativelyCheap,relatively_expensive:relativelyExpensive,stale_data:stale}
    }
  };
}
...
async function buildForUser(client,userId){
  ...
  const rows=(h.data||[]).map(x=>{const sc=sm.get(x.instrument_id)||{},meta=im.get(x.instrument_id)||{};return {...sc,holding:x,meta,sector:meta.sector||"OTHER"};});
  const results=rows.map(row=>{
    const sc=row,holding=row.holding,meta=row.meta,freshness=sc.score_breakdown?.freshness?.status||"MISSING";
    const weight=total>0?(n(holding.current_value)||0)/total*100:0;
    const pnl=holding.pnl_percentage??((n(holding.invested_value)||0)>0?(n(holding.unrealized_pnl)||0)/(n(holding.invested_value)||1)*100:0);
    const peers=rows.filter(x=>x.sector===row.sector&&x.instrument_id!==row.instrument_id);
  ...
  const rank={EXIT:0,REDUCE:1,"HOLD & TRIM":2,WATCH:3,HOLD:4,ACCUMULATE:5,BUY:6};
  results.sort((a,b)=>(rank[a.decision]??99)-(rank[b.decision]??99)||(b.confidence-a.confidence));
...
export async function GET(request){
  try{
    ...
    console.error("Decision engine V3 error:",error);
