...
const ENGINE_VERSION = "decision_engine_v1_1";
...
function decision(score, risk, weight, pnl, freshness, regime) {
  const s=n(score), r=String(risk||"").toUpperCase(), f=String(freshness||"").toUpperCase(), m=String(regime||"").toUpperCase();
...
async function buildForUser(client,userId){
  ...
  const total=(h.data||[]).reduce((a,x)=>a+n(x.current_value),0),regime=mr.data?.[0]?.regime||null;
