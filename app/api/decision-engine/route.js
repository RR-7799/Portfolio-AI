...
function decide(x){
 ...
 if(goodLongTerm&&strongSetup&&risk>=55&&val>=45&&!concentrated&&regime!=="BEAR")return{action:"ACCUMULATE",reason:"Good long-term quality is supported by a strong setup, manageable risk and acceptable valuation.",conviction:"MEDIUM"};
 if(goodLongTerm)return{action:"HOLD",reason:"The long-term thesis remains intact, but the evidence does not currently justify aggressive accumulation.",conviction:"MEDIUM"};
 if(regime==="BEAR"&&final!=null&&final<75)return{action:"HOLD / WAIT",reason:"The thesis is not necessarily broken, but a bearish market regime raises the hurdle for fresh capital.",conviction:"MEDIUM"};
 return{action:"WATCH",reason:"Signals are mixed and do not satisfy the explicit buy, accumulate, hold, reduce or exit gates.",conviction:"LOW"};
}
...
async function buildForUser(client,userId){
 ...
 const results=(h.data||[]).map(row=>{const sc=sm.get(row.instrument_id)||{},meta=im.get(row.instrument_id)||{},weight=total>0?((n(row.current_value)||0)/total*100):0,pnl=row.pnl_percentage??((n(row.invested_value)||0)>0?(n(row.unrealized_pnl)||0)/(n(row.invested_value)||1)*100:null),d=decide({long_term_score:sc.long_term_score,short_term_score:sc.short_term_score,risk_score:sc.risk_score,valuation_score:sc.valuation_score,final_ai_score:sc.final_ai_score,confidence:sc.confidence,data_completeness:sc.data_completeness,freshness:sc.freshness_status,weight,regime});return{instrument_id:row.instrument_id,company_name:meta.company_name||meta.symbol||"Holding",symbol:meta.symbol||null,sector:meta.sector||"OTHER",is_existing_holding:true,portfolio_weight_pct:+weight.toFixed(2),pnl_pct:pnl==null?null:+Number(pnl).toFixed(2),long_term_score:n(sc.long_term_score),long_term_grade:gradeLT(n(sc.long_term_score)),short_term_score:n(sc.short_term_score),short_term_grade:gradeST(n(sc.short_term_score)),risk_score:n(sc.risk_score),valuation_score:n(sc.valuation_score),final_ai_score:n(sc.final_ai_score),final_grade:gradeFinal(n(sc.final_ai_score)),confidence:n(sc.confidence),data_completeness:n(sc.data_completeness),freshness_status:sc.freshness_status||"MISSING",score_version:sc.score_version||"legacy",model_action:sc.action||null,risk_level:sc.risk_level||null,rating:sc.rating||null,decision:d.action,conviction:d.conviction,reason:d.reason,score_breakdown:sc.score_breakdown||{},market_regime:regime};});
