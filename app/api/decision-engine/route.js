...
function decisionV2(input){
  ...
  const growthN=n(growth), profitN=n(profitability), debtN=n(debt), ownershipN=n(ownership), cashN=n(cashflow), valuationN=n(valuation), riskN=n(riskScore);
  const thesis=clamp(avg([growthN,profitN,debtN,cashN],s));
  const quality=clamp(avg([profitN,debtN,cashN,ownershipN],s));
  const opportunity=clamp(avg([s,growthN,valuationN,riskN],s));
  const valuationWeak=valuationN!==null && valuationN<45;
  const strongThesis=thesis>=68 && quality>=60;
  const brokenThesis=thesis<42 && quality<48;
  const criticalRisk=r==="CRITICAL" || (riskN!==null && riskN<30);
  const highRisk=r==="HIGH" || (riskN!==null && riskN<50);
  ...
  else if (strongThesis && valuationN!==null && valuationN>=65 && riskN!==null && riskN>=55 && s>=65 && !stale && m!=="BEAR") { action="ACCUMULATE"; reason="Strong business thesis with attractive valuation and manageable risk"; confidence=87; }
  else if (s>=82 && quality>=65 && risk!=="HIGH" && risk!=="CRITICAL" && !stale && !valuationWeak && m!=="BEAR" && weight<12) { action="BUY"; reason="Strong overall score, quality and opportunity with acceptable portfolio weight"; confidence=90; }
  ...
  return [action,reason,Math.round(confidence),{thesis_score:Math.round(thesis*10)/10,quality_score:Math.round(quality*10)/10,opportunity_score:Math.round(opportunity*10)/10,signals:{strong_thesis:strongThesis,broken_thesis:brokenThesis,critical_risk:criticalRisk,high_risk:highRisk,overweight,severe_drawdown:severeDrawdown,valuation_weak:valuationWeak,stale_data:stale}}];
}
...
async function buildForUser(client,userId){
  const [h,i,s,mr]=await Promise.all([
    ...
    client.from("ai_scores").select("instrument_id,total_score,growth_score,profitability_score,debt_score,ownership_score,cashflow_score,valuation_score,risk_score,action,risk_level,rating,score_breakdown,updated_at,calculated_at"),
