...
const ENGINE_VERSION = "decision_engine_v4_0";
...
function decide(x){
  ...
  const reliable=(conf==null||conf>=60) && !["VERY_STALE","MISSING"].includes(fresh);
  const critical=risk!=null&&risk<40;
  const highRisk=risk!=null&&risk<55;
  const concentrated=weight!=null&&weight>=12;
  const strongCore=lt!=null&&lt>=80;
  const goodCore=lt!=null&&lt>=70;
  const weakCore=lt!=null&&lt<60;
  const strongSetup=st!=null&&st>=80;
  const weakSetup=st!=null&&st<50;
  let action, reason, conviction;

  if(!reliable){ action="WATCH"; reason="Conviction is limited by data completeness, freshness or confidence; do not let an incomplete model force a portfolio action."; conviction="LOW"; }
  ...
  else if(strongCore && (st==null||st>=60) && (risk==null||risk>=55) && (val==null||val>=40)){ action="HOLD"; reason="Long-term quality is strong; current conditions support holding, while the evidence is not strong enough to justify aggressive adding."; conviction="HIGH"; }
  else if(goodCore){ action=weakSetup?"HOLD / WAIT FOR BETTER ENTRY":"HOLD"; reason=weakSetup?"Long-term thesis is good, but current momentum is weak; wait for a better entry rather than confusing business quality with timing.":"Long-term thesis remains constructive, but the current evidence does not justify aggressive accumulation."; conviction="MEDIUM"; }
...
async function buildForUser(client,userId){
  ...
  const total=(h.data||[]).reduce((a,x)=>a+(n(x.current_value)||0),0), regime=mr.data?.[0]?.regime||"NEUTRAL";
  const results=(h.data||[]).map(row=>{
    const sc=sm.get(row.instrument_id)||{}, meta=im.get(row.instrument_id)||{};
    const lt=n(sc.long_term_score??sc.total_score), st=n(sc.short_term_score), risk=n(sc.risk_score), val=n(sc.valuation_score), final=n(sc.final_ai_score??sc.total_score);
...
export async function GET(request){
  try{
    ...
    const persisted=await persistFinalActions(portfolio.decisions,userResult.user.id);
    const decision_counts={}; for(const x of portfolio.decisions) decision_counts[x.decision]=(decision_counts[x.decision]||0)+1;
    ...
    console.error("Decision engine V4 error:",error);
